...
- **Keyboards, UI text, callbacks** are centralized: `src/bot/keyboards/`, `src/bot/ui/` + `src/bot/messages/`, and `src/bot/constants/callbacks.js`. Reuse these rather than inlining strings or callback data.
...
- **Hot paths avoid sync work**: the logger queues writes through an async ordered queue (`flush()` is wired into shutdown in `bootstrap.js`); `storage.updateUserProfile` skips encrypt+write when the profile is unchanged (it runs on every private message); the payment watcher polls merchants in parallel (invoices stay sequential per merchant); the analyze flow sweeps the EVM chains with `Promise.all`; concurrent cold-cache `getPricesEUR` calls coalesce into one CoinGecko request (`src/shared/price.js`); `evm-base.js` caches `getFeeData` for 15 s. Don't reintroduce sync I/O, per-call serialization, or sequential awaits into these paths.
