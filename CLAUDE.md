...
- **`src/bot/handlers/index.js`** — `setupHandlers(bot, storage)` is the composition root. It constructs the shared `WalletService` and `SessionManager`, then calls each feature's `setupXHandlers(...)`. **Dependency injection is positional** — handlers receive `(bot, storage, walletService, sessions)`. Handler groups: start, wallet, keys, send, deposit (receive: asset → network → confirm → address+QR), admin, balance, navigation, exchange, payments, plus the slash commands (`commands/`).
...
- **Keyboards, UI text, callbacks** are centralized: `src/bot/keyboards/`, `src/bot/ui/` + `src/bot/messages/`, and `src/bot/constants/callbacks.js`. Reuse these rather than inlining strings or callback data. Every step of a user journey needs a back button (returning to the **previous step**, not the main menu) and/or a 🎮 Menu escape — terminal screens (deposit QR, lessons) get 🎮 Menu.
- **Telegram output is bounded and awaited**: any message that grows with data (wallet list, analyze report, 💵 Soldes, `/audit`, broadcasts) must go through `splitTelegramMessage`/`sendChunked` (`src/shared/utils/telegram.js` — line-boundary split that keeps `<code>`/`<b>` spans intact; keyboard on the LAST chunk; edit/placeholder modes; tested in `tests/telegram-split.test.js`), and the send must be **awaited** — fire-and-forget sends escape `bot.catch` and resurface as `MESSAGE_TOO_LONG` unhandledRejections. HTML/Markdown escape helpers live in the same file.
- **i18n** (`src/bot/messages/`): locale bundles `fr.js` + `en.js` are merged in `index.js`, which exports `t(lang, 'dot.path', ...args)` (values may be functions taking args) and `resolveLang(hint)` (normalizes a Telegram `language_code`/stored pref to `fr`/`en`, French fallback). Per-request language lives in `ctx.state.lang` (set in `src/bot/handlers/index.js` from `settings.language`, default `fr`); the user toggles it in Settings (`nav.handlers.js`). **Keep `fr.js` and `en.js` key-parity** — every key must exist in both bundles. Reply-keyboard button labels are localized, so new labels must also be wired into the corresponding `bot.hears(...)` matchers.
- **Bot `/learn` lessons** (`src/bot/handlers/commands/info.commands.js`): the user-facing crypto education catalog (26 bilingual lessons — every chain + cross-cutting concepts). Lesson content lives in the i18n bundles under `learn.lessons`; the inline-button menu and direct access (`/learn btc`) are **derived from that object**, so adding a lesson = adding the key to both `fr.js` and `en.js` (nothing to register). `LESSON_ALIASES` maps ~100 vocabulary aliases (`spl`→`sol`, `bip39`→`seed`, accent/separator-insensitive). Routing uses `Object.hasOwn` guards so forged input/callback data can't resolve prototype keys — keep that pattern when touching lesson lookup. This Telegram command is unrelated to the Claude Code `/learn` slash command (see Docs).
- **Hot paths avoid sync work**: the logger queues writes through an async ordered queue (`flush()` is wired into shutdown in `bootstrap.js`); `storage.updateUserProfile` skips encrypt+write when the profile is unchanged (it runs on every private message); the payment watcher polls merchants in parallel (invoices stay sequential per merchant); the analyze flow sweeps the EVM chains with `Promise.all`; concurrent cold-cache `getPricesEUR` calls coalesce into one CoinGecko request (`src/shared/price.js`); `evm-base.js` caches `getFeeData` for 15 s; token `decimals`/`symbol` come from `src/core/tokens.config.js` (locked by `tests/tokens.config.test.js`), never from on-chain calls — this halves the token-scan RPC burst. Don't reintroduce sync I/O, per-call serialization, sequential awaits, or on-chain metadata lookups into these paths.
- **Adding a chain**: create a provider in `src/providers/` (extend `EvmBaseProvider` for EVM chains), register it in `WalletService.chains` (+ `FIRST_WALLET_CHAINS`), add it to `CHAIN_REGISTRY` (`src/shared/chains.js` — drives emojis/labels/derived maps), add RPC defaults in `src/core/config.js`, tokens in `src/core/tokens.config.js`, explorer in `src/shared/explorer.js`, CoinGecko id in `src/shared/coingecko.js` `COIN_IDS` (so it shows a EUR price), a QR logo/label in `src/shared/qr.js` (bundle a PNG under `assets/coin-logos/` if the icon CDN lacks it), the chain-selection keyboard entry, its Trocador network label in `CHAIN_NETWORK` (`src/modules/swap/exchange.service.js`) to make it exchangeable (tokens auto-sync from `TOKEN_CONFIGS`), and a `learn.lessons` entry in both i18n bundles. EVM addresses can't be auto-detected per-network, so they're swept across all EVM chains in the analyze flow (`src/bot/handlers/send/text-input.js`). `src/bot/handlers/commands/info.commands.js` `/list` & `/chains` derive their lists from these registries automatically.
- **Security**: `src/shared/security/` (audit logger — auto-redacts sensitive fields, rate limiter with auto-blacklist), `src/bot/middlewares/` (auth/admin guard, global per-minute rate limit + burst guard, message-length guard, update dedup). There is **no daily-volume cap by design** — transaction throttling relies on the global rate limiter + per-tx dedupe (the unused `dailyVolumeCheck`/`transactionRateLimit`/`sensitiveRateLimit` helpers in `security.middleware.js` are intentionally not wired). The structured logger (`src/shared/logger.js`) auto-redacts sensitive fields **and API-keyed URLs** (`redactUrl` — ethers/fetch errors quote the full request URL, leaking Helius-style `?api-key=` secrets) — log via it, not `console`. The bot auto-leaves any group chat not in `ADMIN_CHAT_ID`. Admin-only `/audit` (alias `/stress`, plus a 🧪 button in the 🔒 Sécurité panel; `src/bot/handlers/admin/audit.js`) runs a **passive, read-only** security report — config check, limiter/blacklist + audit-log stats, wallet-label injection scan, and RPC reachability probes. It mutates nothing; despite the `/stress` name there is no destructive load component.
- **RPC resilience**: `src/shared/rpc/` is the layer every on-chain provider builds on. `RpcManager` (`RpcManager.js`) wraps a list of endpoints with: a **circuit breaker** (opens after `failureThreshold`), health-ranked endpoint ordering (`RpcHealthMonitor.js` — self-healing quarantine; **local token-bucket timeouts don't count as endpoint failures**, only real remote errors do — otherwise healthy keyed endpoints get falsely quarantined), optional **per-endpoint token-bucket rate limiting** (`token-bucket.js`, via the `rps`/`rpsBurst` opts), an optional **TTL + in-flight-dedup request cache** (`request-cache.js`, via `cacheTtlMs`), and an optional `isWrite`/`idempotencyCheck(args, error)` hook (`isWrite` bypasses the cache; the callback can confirm a tx landed before surfacing a broadcast error). Older helpers `rpc-fallback.js`/`resilient-rpc.js` still exist. Each provider constructs **one `RpcManager` per read call-class** as instance fields — `balanceRpc`, `tokenRpc`, `feeRpc`, `utxoRpc` — tuned independently (balances/tokens get short caches + higher `rps`; fee data gets a long cache). **Writes do not go through `RpcManager`**: EVM sends use ethers' own `FallbackProvider` (`getFallbackProvider()`, quorum-1 multi-endpoint), Solana has a bespoke per-endpoint re-sign-and-retry loop with a signature re-check (`_sendWithRetry`/`_checkSignature`), and the UTXO chains broadcast across several explorers (`broadcastTx`). Admin `/audit` reports each manager's `state`/`metrics`/`rateLimitStats`. **Public-endpoint fallback lists** (EVM `fallbackRpcUrls`, UTXO `buildApis`, Solana static fallbacks) are keyless and **load-tested against concurrent bursts** — `getAllTokens` fires every `balanceOf` in parallel, so an endpoint that survives one ping but rate-limits under a burst (meowrpc/cloudflare/llamarpc/1rpc were all dropped for this) is unusable; prefer PublicNode + official RPCs + dRPC/Tenderly. Solana takes a primary RPC plus `SOL_RPC_FALLBACK_URLS` (also overridable via the `solFallbackUrls` vault key); its static keyless fallbacks live in `src/providers/solana.js` — Lava (`solana.lava.build`) is the only public Solana endpoint that survived burst load-testing, so don't swap it out casually; BCH balance/utxo go through Haskoin (it resolves the wallet's legacy addresses, which Bitcore does not).
- **Privacy coins** (Monero, Zcash) can route through a Tor SOCKS proxy (`src/shared/tor-proxy.js`, `TOR_PROXY_URL`).

## Docs

- `docs/architecture.md` — overview in French; **partially stale**: it predates `src/shared/rpc/` (still presents `rpc-fallback.js`/`resilient-rpc.js` as the RPC layer) and `src/bootstrap.js`. Trust this CLAUDE.md over it where they disagree.
- `docs/LEARN.md` — the operator's personal crypto glossary, maintained via the Claude Code `/learn` slash command (`.claude/commands/learn.md`). Append-only; not developer documentation. **Name collision**: the bot also has a Telegram `/learn` command — that one is the user-facing lesson catalog (see Conventions), entirely separate from this file.
